#!/usr/bin/env python3
"""Bring up base101 + cross tower (+ optional dual mod101 arms) in Gazebo Sim.

This is the manipulation overlay's sim launch. It mirrors
base101_gazebo/launch/gazebo.launch.py but loads the overlay URDF
(base101_dual_arm/urdf/base101_dual_arm.xacro) and additionally spawns the
tower / arm controllers and bridges their cameras. The tower / arm controller
params live in this package (controllers.tower.yaml, controllers.arms.yaml) and
are loaded onto the gz_ros2_control controller_manager at spawn time via the
spawner's --param-file, so the core base101 packages stay tower/arm-free.

Args:
    variant   simple           hardware variant (default simple)
    arms      true | false      mount two mod101 arms (needs mod101 underlay)
...
def _setup(context, *args, **kwargs):
    ...
    resource_dirs = [
        ...
        os.path.join(get_package_prefix('base101_tower_description'), 'share'),
    ]
    if arms:
        resource_dirs += [
    ...
    urdf_file = os.path.join(pkg_manip, 'urdf', 'base101_tower.xacro')
    ...
    # Tower + arm controllers: their params are already loaded onto the
    # controller_manager by the gz_ros2_control plugin (base101_tower_control/
    # config/controllers.sim.yaml), so we just spawn them by name.
    post_jsb_spawners = [spawn_diff_drive, Node(
        package='controller_manager',
        executable='spawner',
